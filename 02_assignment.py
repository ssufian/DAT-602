...
def exercise04(more_temperatures, iot_sensor_points, a, b, c, d, e):
    ...
    # ------ Place code below here \/ \/ \/ ------
    # add items in more_temperatures to the temperatures list
    for i in more_temperatures:
        temperatures.append(i)
     # add items in iot_sensor_points (values of dictionary) to the temperatures list
    for i in iot_sensor_points.values(): 
        temperatures.append(i)
        
    #add these arguments to the temperature List
    for j in [a,b,c,d,e]:  
        temperatures.append(j)
        
    #reverse sort the temperatures List
    temperatures.sort(reverse=True)
    
    #Populate only the 5th element from the temperature list into the samples list
...
def exercise10(sentence):
    ...
    # ------ Place code below here \/ \/ \/ ------
    reversedstring =''.join(reversed(sentence)) # .join() method merges all of the characters resulting from the reversed iteration into a new string
    #print(reversedstring) #print the reversed string
    
    # swapcase
    reverse= reversedstring.swapcase()
    # replaced space with underscor
