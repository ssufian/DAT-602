...
def exercise01():
    ...
    df = pd.DataFrame(np.array([1,2,3,4,5,6,7,8,7,8,9,10]).reshape(3,4), columns=[0, 1,2,3])
...
def exercise08():
    ...
    # ------ Place code below here \/ \/ \/ ------
    df = pd.read_csv("http://samplecsvs.s3.amazonaws.com/Sacramentorealestatetransactions.csv ",index_col=False) 

    df.head(5) #peeking at the dataframe
...
def exercise12(n):
    ...
    checkerboard_matrix=np.array([[((i+1)+j)%2 for i in range(2*n)] for j in range(2*n)])
    # ------ Place code above here /\ /\ /\ ------ 
...
def exercise13(n):
    ...
    np.random.seed(1)
    plt.figure();
    s = pd.Series(np.random.randint(0,n,size=(n)), index=pd.date_range('1/1/2010', periods=n))
    s.cumsum()
...
def exercise14(words):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    final_list = list(map(lambda x: len(x), words)) 

    df = pd.DataFrame(final_list, columns = ['String Length']) 
...
def exercise15():
    ...
    df = pd.read_csv("http://samplecsvs.s3.amazonaws.com/Sacramentorealestatetransactions.csv ",index_col=0) 
